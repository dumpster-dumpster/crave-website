...
const reflectiveMaterial = new THREE.MeshStandardMaterial({
  ...
  roughness: 0.15, // Smooth for better reflections
  envMapIntensity: 0.1 // Adjust reflection intensity
});
...
function animate() {
  ...
  // Animate the scale of the sphere to make it more liquid-like
  const scale = 1 + 0.1 * Math.sin(time * 3); // Oscillate scale between 0.9 and 1.1
...
// Update toggleOverlayAndText to show for 5 seconds and repeat every 5 seconds
function toggleOverlayAndText() {
  setInterval(() => {
    overlay.style.display = 'block';
    let isTextVisible = false;

    // Flash the text on and off during the 5 seconds
    ...
    setTimeout(() => {
      overlay.style.display = 'none';
      finalExamText.style.display = 'none';
      clearInterval(flashInterval); // Stop flashing after 5 seconds
